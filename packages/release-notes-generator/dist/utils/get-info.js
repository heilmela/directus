...
async function getInfo(changesets) {
    ...
    for (const { summary, notice, commit, releases } of changesets.values()) {
        ...
        for (const { type, name } of releases) {
            if (name === constants_js_1.MAIN_PACKAGE || !summary) {
                continue;
            }
            if (isUntypedPackage(name)) {
                const untypedPackageName = constants_js_1.UNTYPED_PACKAGES[name];
            ...
            const typeTitle = constants_js_1.TYPE_MAP[type];
    ...
    types.sort((0, sort_js_1.sortByObjectValues)(constants_js_1.TYPE_MAP, 'title'));
    for (const { packages } of types) {
        packages.sort((0, sort_js_1.sortByExternalOrder)(constants_js_1.PACKAGE_ORDER, 'name'));
    }
    untypedPackages.sort((0, sort_js_1.sortByObjectValues)(constants_js_1.UNTYPED_PACKAGES, 'name'));
    return { types, untypedPackages, notices };
}
exports.getInfo = getInfo;
function isUntypedPackage(name) {
    return Object.prototype.hasOwnProperty.call(constants_js_1.UNTYPED_PACKAGES, name);
}
