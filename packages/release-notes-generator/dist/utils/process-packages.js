...
async function processPackages() {
    ...
    for (const localPackage of workspacePackages) {
        ...
        if (name === constants_js_1.MAIN_PACKAGE) {
            mainVersion = version;
        }
    ...
    const packageVersions = Array.from(packageVersionMap, ([name, version]) => ({
        ...
        .filter(({ name }) => ![constants_js_1.MAIN_PACKAGE, ...Object.keys(constants_js_1.UNTYPED_PACKAGES)].includes(name))
        .sort((0, sort_js_1.sortByExternalOrder)(constants_js_1.PACKAGE_ORDER, 'name'));
